...
def gen_spark(data, key, count, perc=False):
    ...
    res = ""
    for v in vals:
        res += util.get_bar(100.0* (v - min_val) / (max_val - min_val), not perc)
    return res
...
def gen_dict(report):
    ...
    data['gprecipH5'] = gen_spark(report['hourly']['data'], 'precipProbability', 5, True)
    data['gprecipH10'] = gen_spark(report['hourly']['data'], 'precipProbability', 10, True)
    data['gprecipH15'] = gen_spark(report['hourly']['data'], 'precipProbability', 15, True)
    data['gprecipH24'] = gen_spark(report['hourly']['data'], 'precipProbability', 24, True)
    return data
